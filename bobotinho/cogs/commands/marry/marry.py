...
async def func(ctx, arg: str):
    ...
    elif await models.Wedding.exists_or(user_1_id=ctx.author.name, user_2_id=ctx.author.name):
        ctx.response = "traição é inaceitável, ao menos se divorcie antes de partir pra outra"
    elif await models.Wedding.exists_or(user_1_id=name, user_2_id=name):
        ctx.response = f"controle seu desejo por pessoas casadas, @{name} já está em um compromisso"
    elif (cookie := await models.Cookie.get_or_none(user_id=ctx.author.name)) and cookie.stocked >= 100:
