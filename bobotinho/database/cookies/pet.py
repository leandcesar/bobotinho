...
class Pet(Base):
    user = fields.ForeignKeyField("cookies.Cookie", to_field="name")
    ...
    async def find(cls, user: str, name: str = None, specie: str = None):
        return (
            await cls.filter(user_id=user, name=name).first()
            or await cls.filter(user_id=user, specie=specie).first()
        )
